...
var Cartesian = /** @class */ (function () {
    ...
    // GRAPH PLOT
    Cartesian.prototype.addGraph2D = function (exp, animDuration) {
        ...
        this.graphs.push({ exp: exp, color: color, animDuration: animDuration, currentStatus: animDuration });
    };
    Cartesian.prototype.removeGraph2D = function (exp) {
        this.graphs = this.graphs.filter(function (graph) { return graph.exp !== exp; });
    };
    Cartesian.prototype.plotGraph2D = function (graph) {
        var exp = graph.exp, color = graph.color, animDuration = graph.animDuration, currentStatus = graph.currentStatus;
        ...
        var currentPoint = animDuration - currentStatus;
        if (animDuration <= 0)
            currentPoint = 1;
        else {
        ...
        var limit = results.length * currentPoint;
        ctx.beginPath();
